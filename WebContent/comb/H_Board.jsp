...
<body>
	<!-- 게시글 페이지수 -->
<!--총 페이지수 -->
<% int page_show=10; %><!-- 게시판이 목록이 보여지는 갯수설정 -->
  ...
  <ul class="w3-navbar w3-center w3-round-xlarge w3-margin-bottom w3-hide-medium w3-hide-small" style="width:95%; margin:0px 2.5% 0px 2.5% ">
	...
	<ul class="w3-navbar w3-center w3-round-xlarge w3-margin-bottom w3-hide-medium w3-hide-small" style="width:95%; margin:0px 2.5% 0px 2.5% ">
      	...
      	<li class="w3-white" style="width:30%">
      		<a href="HoneyControl?action=viewlist&from_page=<%=from_page %>&from=<%=from %>&list_n=<%=event.getList_n() %>">
      			<%=event.getList_title() %>
      		</a>
      	</li>
      	<!--작성자-->
        ...
        <li style="width:11%">
        	<a href="HoneyControl?action=updateGood&from=<%=from %>&from_page=<%=from_page %>&list_n=<%= event.getList_n() %>&good=<%= event.getList_good() %>" class="w3-amber w3-hover-yellow"><%=event.getList_good() %></a>
        </li>
        <li style="width:11%">
        	<a href="HoneyControl?action=updateBad&from=<%=from %>&from_page=<%=from_page %>&list_n=<%= event.getList_n() %>&bad=<%= event.getList_bad() %>" class="w3-brown w3-hover-dark-gray"><%=event.getList_bad() %></a>
        </li>
   	</ul>  
  		...
  		<li style="width:100%">
    		<ul class="w3-navbar w3-center">
    			...
    			<li class="w3-white" style="width:38%">
            		<a href="HoneyControl?action=viewlist&from_page=<%=from_page %>&from=<%=from %>&list_n=<%=event.getList_n() %>" class="w3-padding-16">
                		<%=event.getList_title() %>
                	</a>
            	</li>
  		...
  		<li style="width:100%">
    		<ul class="w3-navbar  w3-khaki w3-center">
    			<li class="w3-amber w3-small" style="width:50%">
            		<a href="HoneyControl?action=updateGood&from=<%=from %>&from_page=<%=from_page %>&list_n=<%= event.getList_n() %>&good=<%= event.getList_good() %>" class="w3-hover-yellow">
                		<%=event.getList_good() %>
                	</a>
            	</li>
    			<li class="w3-brown w3-small" style="width:50%">
            		<a href="HoneyControl?action=updateBad&from=<%=from %>&from_page=<%=from_page %>&list_n=<%= event.getList_n() %>&bad=<%= event.getList_bad() %>"  class="w3-hover-dark-gray">
                		<%=event.getList_bad() %>
                	</a>
            	</li>
 			</ul>
	...
	<ul class="w3-pagination">
		<% if(view_a-1>0){%>
			<li><a href="?action=<%=from %>&page_n=<%=view_a-1%>">&laquo;</a></li>
		<%} %>
		<%for(int s=view_a;s<view_a+remain_a;s++) {
			if(s==page_n){			%>
				<li><a href="?action=<%=from %>&page_n=<%=s%>" class="w3-green"><%=s%></a></li>
		<%}else{ %>
				<li><a href="?action=<%=from %>&page_n=<%=s%>" class="w3-white w3-hover-red"><%=s%></a></li>
		<%}} %>
		<% if(all_p-view_a>10){%>
				<li><a href="?action=<%=from %>&page_n=<%=view_a+10%>">&raquo;</a></li> 
		<%} %>
	</ul>
